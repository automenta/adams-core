...
/**
 <!-- globalinfo-start -->
 * Applies a report filter that is referenced via its global setup name (see 'NamedSetups').
 * <p/>
 <!-- globalinfo-end -->
 *
 <!-- options-start -->
...
public class NamedSetup<T extends DataContainer>
  ...
  protected AbstractReportFilter getActualScheme() {
    if (m_ActualScheme == null) {
      m_ActualScheme = (AbstractReportFilter) m_Setup.getSetup();
      if (m_ActualScheme == null)
  ...
  protected T processData(T data) {
    return (T) getActualScheme().filter(data);
  }
}
