...
/**
 <!-- globalinfo-start -->
 * Applies an outlier detector that is referenced via its global setup name (see 'NamedSetups').
 * <p/>
 <!-- globalinfo-end -->
 *
 <!-- options-start -->
...
public class NamedSetup<T extends DataContainer>
  ...
  /** the actual scheme. */
  protected AbstractOutlierDetector<T> m_ActualScheme;
  ...
  public String globalInfo() {
    return "Applies an outlier detector that is referenced via its global setup name (see 'NamedSetups').";
  }
   ...
   */
  protected AbstractOutlierDetector<T> getActualScheme() {
    if (m_ActualScheme == null) {
      m_ActualScheme = (AbstractOutlierDetector<T>) m_Setup.getSetup();
      if (m_ActualScheme == null)
  ...
  protected List<String> processData(T data) {
    return getActualScheme().detect(data);
  }
}
