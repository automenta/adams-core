...
/**
 <!-- globalinfo-start -->
 * Applies a baseline correction scheme that is referenced via its global setup name (see 'NamedSetups').
 * <p/>
 <!-- globalinfo-end -->
 *
 <!-- options-start -->
...
public class NamedSetup<T extends DataContainer>
  ...
  protected AbstractBaselineCorrection getActualScheme() {
    if (m_ActualScheme == null) {
      m_ActualScheme = (AbstractBaselineCorrection) m_Setup.getSetup();
      if (m_ActualScheme == null)
  ...
  protected T processData(T data) {
    return (T) getActualScheme().correct(data);
  }
}
