...
/**
 * ArrayRootRelativeSquaredError.java
...
/**
 <!-- globalinfo-start -->
 * Calculates the root relative squared error (RRSE) between the first array (actual) and the remaining arrays (predicted). The arrays must be numeric, of course.
 * <p/>
 <!-- globalinfo-end -->
 *
 <!-- options-start -->
...
public class ArrayRootRelativeSquaredError<T extends Number>
  ...
  public String globalInfo() {
    return
        "Calculates the root relative squared error (RRSE) between the first array "
  ...
  protected StatisticContainer doCalculate() {
    ...
    for (i = 1; i < size(); i++) {
      ...
      result.setCell(0, i - 1, StatUtils.rrse(get(0), get(i)));
    }
