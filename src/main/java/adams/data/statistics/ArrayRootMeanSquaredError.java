...
/**
 * ArrayRootMeanSquaredError.java
...
/**
 <!-- globalinfo-start -->
 * Calculates the root mean squared error (RMSE) between the first array (actual) and the remaining arrays (predicted). The arrays must be numeric, of course.
 * <p/>
 <!-- globalinfo-end -->
 *
 <!-- options-start -->
 ...
 */
public class ArrayRootMeanSquaredError<T extends Number>
  ...
  public String globalInfo() {
    return
        "Calculates the root mean squared error (RMSE) between the first array "
  ...
  protected StatisticContainer doCalculate() {
    ...
    for (i = 1; i < size(); i++) {
      result.setHeader(i - 1, "RMSE 1-" + (i+1));
      result.setCell(0, i - 1, StatUtils.rmse(get(0), get(i)));
    }
