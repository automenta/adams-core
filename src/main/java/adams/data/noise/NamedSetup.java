...
/**
 <!-- globalinfo-start -->
 * Applies a denoiser that is referenced via its global setup name (see 'NamedSetups').
 * <p/>
 <!-- globalinfo-end -->
 *
 <!-- options-start -->
...
public class NamedSetup<T extends DataContainer>
  extends AbstractDatabaseConnectionDenoiser<T> {

  /** for serialization. */
  private static final long serialVersionUID = 6853534047304907639L;

  /** the name of the setup to load. */
  protected adams.core.NamedSetup m_Setup;

  /** the actual scheme. */
  protected AbstractDenoiser<T> m_ActualScheme;
  ...
  public String globalInfo() {
    return "Applies a denoiser that is referenced via its global setup name (see 'NamedSetups').";
  }
   ...
   */
  protected AbstractDenoiser getActualScheme() {
    if (m_ActualScheme == null) {
      m_ActualScheme = (AbstractDenoiser<T>) m_Setup.getSetup();
      if (m_ActualScheme == null)
  ...
  protected T processData(T data) {
    return (T) getActualScheme().denoise(data);
  }
}
