 ...
 * 
 * Multiple sets of keywords can be defined with a properties file, each with
 * different colors. The first set of keywords is listed under the heading
 * 'Keywords', with the corresponding color 'KeywordColor'. All subsequent sets
 * use the same headings as the first but with their number attached:
 * 'Keywords2', 'KeywordColor2'; 'Keywords3', KeywordColor3'; etc.
 * 
 * @author Rob Camick (primary author; java sun forums user 'camickr')
...
public class SyntaxDocument
  ...
  protected void setup(Properties props) {
    ...
    setSingleLineComment(props.getBoolean("SingleLineComment", true));
    setSingleLineCommentStart(props.getProperty("SingleLineCommentStart", "//"));
    setMultiLineComment(props.getBoolean("MultiLineComment", false));
    ...
    setAddMatchingEndBlocks(props.getBoolean("AddMatchingBlockEnd", false));
    setUseBlanks(props.getBoolean("UseBlanks", false));
    setCaseSensitive(props.getBoolean("CaseSensitive", true));

    for (int i = 1;; i++) {
      String keywords;
      String keywordId = "Keywords";
      String keywordColorId = "KeywordColor";
      if (i > 1) {
	keywordColorId += i;
	keywordId += i;
      }
	...
	break;
      ...
      addKeywords(keywords.trim().replaceAll(" ", "").split(","), style);
      setAttributeColor(style, props.getColor(keywordColorId, Color.BLUE));
    }

    setTabs(props.getInteger("Tabs", 2));
    setAttributeColor(DEFAULT_NORMAL,
	props.getColor("ForegroundColor", Color.BLACK));
    setAttributeColor(DEFAULT_COMMENT,
	props.getColor("CommentColor", Color.GRAY));
    setAttributeColor(DEFAULT_STRING, props.getColor("StringColor", Color.RED));
    setBackgroundColor(props.getColor("BackgroundColor", Color.WHITE));
  ...
  protected void applyHighlighting(String content, int line)
    ...
    if (getSingleLineComment())
      index = content.indexOf(getSingleLineCommentStart(), startOffset);
  ...
  protected int getOtherToken(String content, int startOffset, int endOffset) {
	...
	break;
      if (isQuoteDelimiter(content.substring(endOfToken, endOfToken + 1)))
	break;
      endOfToken++;
    }
    ...
    //return the position of the delimiter which ended this token because if 
    //it was a quote delimiter we still have to parse it.
    return endOfToken;
  }
  ...
  public void setSingleLineComment(boolean value) {
    m_SingleLineComment = value;
  }

  /**
   * Returns whether single-line comments are enabled.
   * 
   * @return true if single-line comments are enabled
   */
