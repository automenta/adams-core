...
public class BaseTimeEditor
  ...
  protected JComponent createCustomEditor() {
	...
	if (index > 1) {
	  BaseTime time = new BaseTime((String) m_ComboBoxPlaceholders.getSelectedItem());
	...
	int index = m_ComboBoxPlaceholders.getSelectedIndex();
	if (index == 0)
	  setValue(new BaseTime(m_TextCustom.getText()));
	else if (index == 1)
	  setValue(new BaseTime(m_Calendar.getDate()));
	else
	  setValue(new BaseTime((String) m_ComboBoxPlaceholders.getSelectedItem()));
