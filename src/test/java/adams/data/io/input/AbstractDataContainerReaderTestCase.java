...
public abstract class AbstractDataContainerReaderTestCase<A extends AbstractDataContainerReader, D extends DataContainer>
  ...
  /**
   * Loads the data to process. If the reader reads from a directory, then
   * the tmp directory will be used as input. In that case, the reader has
   * to be set up correctly to find its data in the tmp directory.
   *
  ...
  protected List<D> load(String filename, A reader) {
    ...
    if (reader.isInputFile())
      ...
      reader.setInput(new TmpFile());
    result = new ArrayList<>(reader.read());
  ...
  protected boolean save(List<D> data, String filename, int[] ignored) {
    ...
    for (i = 0; i < data.size(); i++) {
      m_TestHelper.save(data.get(i), filename);
